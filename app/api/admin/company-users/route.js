...
import { decryptUserPassword, encryptUserPassword } from '@/lib/password-vault'
    ...
    await getFirebaseAdminDb().collection(USERS_COLLECTION).doc(authUser.uid).set(
      {
        ...
        passwordVault: encryptUserPassword(payload.email, payload.password),
        active: true,
...
export async function PUT(request) {
  try {
    ...
    if (payload.password) {
      profileUpdates.passwordVault = encryptUserPassword(payload.email, payload.password)
    }
