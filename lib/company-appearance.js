...
export function normalizeCompanyAppearance(value = {}) {
  ...
  return {
    ...
    brightness: clamp(
      Number.isFinite(Number(source.brightness ?? source.contrast))
        ? (source.brightness ?? source.contrast)
        : 100,
      50,
      150
    ),
...
export function resolveCompanyChrome(value = {}) {
  ...
  if (appearance.advancedEnabled) {
    ...
    const brightnessFactor = appearance.brightness / 100
    rgb = {
      r: rgb.r * brightnessFactor,
      g: rgb.g * brightnessFactor,
      b: rgb.b * brightnessFactor,
    }
  }
