...
function normalizeDashboardFilter(filter = {}) {
  ...
  const fallbackField = findDashboardFieldByStorage(String(filter.table || '').trim(), String(filter.column || '').trim())
  const resolvedField = directField || fallbackField

  return {
    id: String(filter.id || `${filter.table || 'filtro'}-${filter.column || 'coluna'}-${filter.operator || 'is'}`).trim(),
    source: ['standard', 'table'].includes(filter.source) ? filter.source : resolvedField ? 'standard' : 'table',
