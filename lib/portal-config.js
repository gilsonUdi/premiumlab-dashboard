...
export function buildDefaultUserPermissions(company = {}) {
  ...
  const powerBiEnabled =
    company.powerBiEnabled === true &&
    (Boolean(company.powerBiReportId && company.powerBiWorkspaceId) || Boolean(company.powerBiEmbedUrl))
...
export function getAllowedPowerBiPages(company, permissions) {
  const normalizedPermissions = normalizeUserPermissions(permissions, company)
