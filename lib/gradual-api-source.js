...
function resolveRouteSteps(routes = [], expected, now) {
  return (routes || []).map((route, index) => {
    const label = normalizeText(route.warehouseCode || route.locationCode || route.warehouseDescription || `ETP${index + 1}`)
...
function normalizeOrderRow(order = {}, routesByOrderId = new Map(), now = new Date()) {
  ...
  return {
    ...
    clicodigo: String(order.clicodigo || order.customerId || ''),
    clinome: normalizeText(order.clinome || order.customerName || 'Cliente nao informado'),
