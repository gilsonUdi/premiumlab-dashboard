...
const MAX_LIMIT_CAP = Math.max(1, Number(process.env.GRADUAL_API_LIMIT_CAP || 200))
const MAX_SCAN_WINDOW_CAP = Math.max(100, Number(process.env.GRADUAL_API_SCAN_WINDOW_CAP || 2000))
...
function buildRetryCandidates(primaryUrl) {
  ...
  const fallbacks = [
    ...
    [Math.min(limit, 50), Math.min(scanWindow, 500)],
    [Math.min(limit, 30), Math.min(scanWindow, 300)],
  ]
