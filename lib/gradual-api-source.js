...
const MAX_LIMIT_CAP = Math.max(1, Number(process.env.GRADUAL_API_LIMIT_CAP || 2000))
const MAX_SCAN_WINDOW_CAP = Math.max(100, Number(process.env.GRADUAL_API_SCAN_WINDOW_CAP || 20000))
const FETCH_TIMEOUT_MS = Math.max(5000, Number(process.env.GRADUAL_API_FETCH_TIMEOUT_MS || 45000))

function normalizeText(value) {
  return String(value || '').trim()
}

function normalizeCellAbbreviation(value) {
...
function resolveCustomerName(order = {}) {
  const customerInfo = order.customer || order.client || order.buyer || {}
  ...
  for (const [key, value] of Object.entries(customerInfo)) {
    ...
    if (!/name|descricao|description|razao|social|fantasia/i.test(key)) continue
...
function resolveSellerCode(order = {}) {
  const sellerInfo = order.seller || order.vendedor || order.vendor || order.employee || order.representative || {}
  ...
  for (const [key, value] of Object.entries(sellerInfo)) {
    if (!/id|codigo|code|seller|vendor|employee|func/i.test(key)) continue
    const candidate = normalizeText(value)
    if (candidate) return candidate
  }
...
function resolveSellerName(order = {}) {
  const sellerInfo = order.seller || order.vendedor || order.vendor || order.employee || order.representative || {}
  const directName = normalizeText(
    order.vendedorNome ||
    order.sellerName ||
    ...
    order.funcionarioNome ||
    order.representativeName ||
    order.salesmanName ||
    sellerInfo.name ||
    sellerInfo.fullName ||
    sellerInfo.tradeName ||
  ...
  for (const [key, value] of Object.entries(sellerInfo)) {
    if (typeof value !== 'string') continue
    if (!/name|nome|description|descricao|seller|vendor|employee|represent/i.test(key)) continue
    const candidate = normalizeText(value)
    if (candidate) return candidate
  }
...
function resolveRouteSteps(routes = [], expected, now) {
  return (routes || []).map((route, index) => {
    ...
    const cellCode = normalizeText(route.warehouseCode || route.locationCode || route.code || `${index + 1}`)
    const shortCellName = normalizeCellAbbreviation(
      route.warehouseDescription ||
      route.locationDescription ||
      route.currentCell ||
      route.location ||
    ...
    const state = finishedAt
      ? (expected && finishedAt > expected ? 'delayed' : 'completed')
      : 'pending'

    return {
      ordem: index + 1,
      label: `${cellCode}-${shortCellName}`,
...
function normalizeOrderRow(order = {}, routesByOrderId = new Map(), now = new Date()) {
  ...
  const sellerCode = resolveSellerCode(order)
  const sellerName = resolveSellerName(order)
  ...
  return {
    ...
    caixa: normalizeText(order.caixa || order.box || order.boxNumber || '-') || '-',
    roteiro,
    ...
    vendedorCodigo: sellerCode,
    vendedorNome: sellerName || (sellerCode ? `Cod. ${sellerCode}` : 'Nao informado'),
    emittedDate,
    deliveredDate,
  }
}
    ...
    filteredOrders = filteredOrders.filter(order => {
      if (field === 'pedcodigo') {
        return values.some(value => (
          matchesFilter(order.pedcodigo, { operator: 'is', value }) ||
          matchesFilter(order.pedidoId, { operator: 'is', value })
        ))
      }

      if (field === 'clinome') {
        return values.some(value => (
          matchesFilter(order.clinome, { operator: 'contains', value }) ||
          matchesFilter(order.clicodigo, { operator: 'is', value })
        ))
      }

      return values.some(value => matchesFilter(order[field], { operator: 'is', value }))
    })
  }
...
function buildApiUrl(settings, searchParams) {
  ...
  const requestedLimit = Number(settings.gradualApiLimit) || DEFAULT_LIMIT
  const requestedScanWindow = Number(settings.gradualApiScanWindow) || DEFAULT_SCAN_WINDOW
  url.searchParams.set('limit', String(Math.max(1, Math.min(requestedLimit, MAX_LIMIT_CAP))))
  url.searchParams.set('scanWindow', String(Math.max(1, Math.min(requestedScanWindow, MAX_SCAN_WINDOW_CAP))))
...
function buildRetryCandidates(primaryUrl) {
  const base = new URL(primaryUrl.toString())
  const limit = Math.max(1, Number(base.searchParams.get('limit')) || DEFAULT_LIMIT)
  const scanWindow = Math.max(1, Number(base.searchParams.get('scanWindow')) || DEFAULT_SCAN_WINDOW)

  const candidates = [[limit, scanWindow]]

  const fallbacks = [
    [Math.min(limit, 500), Math.min(scanWindow, 5000)],
    [Math.min(limit, 200), Math.min(scanWindow, 2000)],
    [Math.min(limit, 100), Math.min(scanWindow, 1000)],
  ]

  for (const [nextLimit, nextWindow] of fallbacks) {
    if (!candidates.some(([l, w]) => l === nextLimit && w === nextWindow)) {
      candidates.push([Math.max(1, nextLimit), Math.max(1, nextWindow)])
    }
  }

  return candidates.map(([nextLimit, nextWindow]) => {
    const candidate = new URL(base.toString())
    candidate.searchParams.set('limit', String(nextLimit))
    candidate.searchParams.set('scanWindow', String(nextWindow))
    return candidate
  })
}

async function fetchGradualPayload(primaryUrl, headers = {}) {
  const candidates = buildRetryCandidates(primaryUrl)
  let lastError = null
  ...
  for (const candidateUrl of candidates) {
    try {
      ...
      if (!response.ok) {
        const message = `API Gradual respondeu ${response.status}: ${responseText || response.statusText}`
      ...
      try {
        ...
        return { payload, usedUrl: candidateUrl.toString() }
      } catch {
      ...
      const isTimeout = error?.name === 'AbortError' || /timed out|timeout/i.test(text)
      if (isTimeout) {
        lastError = new Error('A API Gradual demorou para responder (timeout).')
        continue
      }

      if (/API Gradual respondeu 50[234]/i.test(text)) {
        lastError = error
        continue
      }
  ...
  throw lastError || new Error('Falha ao consultar a API Gradual.')
}
...
export async function buildGradualApiDashboardPayload({
  ...
  const { payload, usedUrl } = await fetchGradualPayload(
    url,
    gradualApiKey ? { 'x-gs-api-key': gradualApiKey } : {}
  )
