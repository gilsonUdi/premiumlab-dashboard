...
export const ADMIN_PREVIEW_USER_HEADER = 'x-gs-admin-preview-user'
...
function normalizeCompany(company) {
  ...
  const lossFinalityCodes = Array.isArray(company.lossFinalityCodes)
    ? company.lossFinalityCodes
    : String(company.lossFinalityCodes || '')
        .split(',')
  ...
  return {
    ...
    hasGradualApiKey: Boolean(company.hasGradualApiKey),
...
function buildAdminPreviewSession(baseSession, company, user = null) {
  const isUserPreview = Boolean(user?.uid)

  return {
    ...baseSession,
    type: 'admin',
    adminCompanyPreview: true,
    adminPreviewUserId: isUserPreview ? user.uid : '',
    adminPreviewUserName: isUserPreview ? user.name || user.email || 'Usuario' : 'Administrador',
    adminPreviewUserEmail: isUserPreview ? user.email || '' : '',
    companyId: company.id,
    companySlug: company.slug,
    companyName: company.name,
    permissions: isUserPreview ? normalizeUserPermissions(user.permissions, company) : null,
  }
}

function preserveAdminPreviewSession(session) {
  if (typeof window === 'undefined' || session?.type !== 'admin') return session

  const currentSession = loadPortalSession()
  if (!currentSession?.adminCompanyPreview || currentSession.uid !== session.uid) return session

  return {
    ...session,
    adminCompanyPreview: true,
    adminPreviewUserId: currentSession.adminPreviewUserId || '',
    adminPreviewUserName: currentSession.adminPreviewUserName || 'Administrador',
    adminPreviewUserEmail: currentSession.adminPreviewUserEmail || '',
    companyId: currentSession.companyId || '',
    companySlug: currentSession.companySlug || '',
    companyName: currentSession.companyName || '',
    permissions: currentSession.permissions || null,
  }
}
...
export async function getPortalAuthHeaders() {
  const token = await getAuthToken()
  const headers = token ? { Authorization: `Bearer ${token}` } : {}
  const session = loadPortalSession()

  if (session?.type === 'admin' && session.adminCompanyPreview && session.adminPreviewUserId) {
    headers[ADMIN_PREVIEW_USER_HEADER] = session.adminPreviewUserId
  }
...
export async function getCurrentPortalSession() {
  ...
  const session = preserveAdminPreviewSession(buildSession(profile, authUser))
...
export function startAdminCompanyPreview(company, user = null) {
  const currentSession = loadPortalSession()
  if (!currentSession || currentSession.type !== 'admin') return null

  const previewSession = buildAdminPreviewSession(currentSession, company, user)
  savePortalSession(previewSession)
...
export async function ensurePremiumLabTenant() {
  ...
  await authorizedAdminRequest('/api/admin/companies', {
    ...
    body: JSON.stringify({
      ...
      dashboardFilters: existingCompany?.dashboardFilters || PREMIUM_COMPANY.dashboardFilters,
      dashboardVisualFilters: existingCompany?.dashboardVisualFilters || PREMIUM_COMPANY.dashboardVisualFilters,
      dashboardDataSource: existingCompany?.dashboardDataSource || PREMIUM_COMPANY.dashboardDataSource,
      gradualApiUrl: existingCompany?.gradualApiUrl || PREMIUM_COMPANY.gradualApiUrl,
      gradualApiSource: existingCompany?.gradualApiSource || PREMIUM_COMPANY.gradualApiSource,
      gradualApiCompanyIds: existingCompany?.gradualApiCompanyIds || PREMIUM_COMPANY.gradualApiCompanyIds,
      gradualApiScanWindow: existingCompany?.gradualApiScanWindow || PREMIUM_COMPANY.gradualApiScanWindow,
      gradualApiStartOrderId: existingCompany?.gradualApiStartOrderId || PREMIUM_COMPANY.gradualApiStartOrderId,
      gradualApiLimit: existingCompany?.gradualApiLimit || PREMIUM_COMPANY.gradualApiLimit,
      orderCompletionRules: existingCompany?.orderCompletionRules || PREMIUM_COMPANY.orderCompletionRules,
      limitByCompanyCodeEnabled: existingCompany?.limitByCompanyCodeEnabled ?? PREMIUM_COMPANY.limitByCompanyCodeEnabled,
      companyCodeFilter: existingCompany?.companyCodeFilter || PREMIUM_COMPANY.companyCodeFilter,
...
export async function upsertCompany(state, payload) {
  ...
  await authorizedAdminRequest('/api/admin/companies', {
    ...
    body: JSON.stringify({
      ...
      gradualApiKey: payload.gradualApiKey || '',
      supabaseEnabled: portalSettings.supabaseEnabled,
      externalDashboardUrl: portalSettings.externalDashboardUrl,
      powerBiEnabled: portalSettings.powerBiEnabled,
      powerBiEmbedUrl: portalSettings.powerBiEmbedUrl,
