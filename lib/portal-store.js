...
export async function ensurePremiumLabTenant() {
  ...
  await authorizedAdminRequest('/api/admin/companies', {
    ...
    body: JSON.stringify({
      ...
      externalDashboards: existingCompany?.externalDashboards || PREMIUM_COMPANY.externalDashboards,
      ...
      dashboardFeedingModel: existingCompany?.dashboardFeedingModel || existingCompany?.dashboardDataSource || PREMIUM_COMPANY.dashboardFeedingModel,
      ...
      apiCancellationCodes: existingCompany?.apiCancellationCodes || PREMIUM_COMPANY.apiCancellationCodes,
      portalAppearance: existingCompany?.portalAppearance || PREMIUM_COMPANY.portalAppearance,
...
export async function upsertCompany(state, payload) {
  ...
  const normalized = normalizeCompany({
    ...
    limitByCompanyCodeEnabled: portalSettings.limitByCompanyCodeEnabled,
    companyCodeFilter: portalSettings.companyCodeFilter,
    apiCancellationCodes: portalSettings.apiCancellationCodes,
    portalAppearance: portalSettings.portalAppearance,
  })
...
export async function deleteCompany(state, companyId) {
  ...
  if (company.isPremiumLab) {
    throw new Error('A Premium Lab não pode ser excluída por esta tela.')
  }
