...
export const POWER_BI_PAGE_VISIT_COLLECTION = 'powerBiPageVisits'
...
async function createOrTouchSession({ sessionId, user, company, reportKey, reportLabel = '' }) {
  const db = getFirebaseAdminDb()
  const now = new Date()
  ...
  await db.runTransaction(async transaction => {
    ...
    transaction.set(
      ref,
      {
        ...
        reportKey,
        reportLabel: String(reportLabel || existing?.reportLabel || reportKey),
        isAdminAccess: Boolean(user.isAdminAccess),
        isAdminPreview: Boolean(user.isAdminPreview),
        excludeFromUsageMetrics: Boolean(user.isAdminAccess),
        adminUserId: String(user.adminUserId || ''),
        adminUserEmail: String(user.adminUserEmail || ''),
        adminUserName: String(user.adminUserName || ''),
...
export async function preparePowerBiDashboardAccess({ sessionId, user, company, reportKey, reportLabel = '' }) {
  if (!isPowerBiCapacityManagementConfigured()) {
    return { enabled: false, ready: true, sessionId: '' }
  }

  const id = await createOrTouchSession({ sessionId, user, company, reportKey, reportLabel })
  const context = {
    companyId: company.id,
    companyName: company.name,
    userId: user.uid,
...
function normalizeAutomationPart(value, fallback) {
...
function getPowerBiAutomationContext({ executionId, source = 'morning-call', tenant = '' }) {
  const normalizedExecutionId = normalizeAutomationPart(executionId, '')
  if (!normalizedExecutionId) throw new Error('Identificador da execucao da automacao nao informado.')

  const normalizedSource = normalizeAutomationPart(source, 'morning-call')
  const normalizedTenant = normalizeAutomationPart(tenant, 'shared')
  const sessionId = `automation-${normalizedSource}-${normalizedExecutionId}`

  return {
    sessionId,
    reportKey: `automation-${normalizedSource}`,
    reportLabel: `Automacao ${normalizedSource}`,
    company: {
      id: `automation-${normalizedTenant}`,
      slug: `automation-${normalizedTenant}`,
      name: tenant ? `Automacao ${tenant}` : 'Automacao Power BI',
    },
    user: {
      uid: `automation-${normalizedSource}`,
      email: '',
      name: `Automacao ${normalizedSource}`,
      isAdminAccess: true,
...
export async function acquirePowerBiAutomationLease({ executionId, source, tenant }) {
  const context = getPowerBiAutomationContext({ executionId, source, tenant })
  const result = await preparePowerBiDashboardAccess(context)
  return {
    ...result,
...
export async function releasePowerBiAutomationLease({ executionId, source, tenant, reason = 'automation_completed' }) {
  const context = getPowerBiAutomationContext({ executionId, source, tenant })
...
export async function heartbeatPowerBiDashboardSession({ sessionId, user, company, reportKey, pageName = '', pageLabel = '', pageSequence = 0 }) {
  ...
  const now = new Date()
  const settings = getPowerBiCapacitySettings()
  const normalizedPageName = String(pageName || '').trim().slice(0, 200)
  const normalizedPageLabel = String(pageLabel || pageName || '').trim().slice(0, 200)
  const normalizedPageSequence = Number.isFinite(Number(pageSequence)) ? Math.max(0, Math.floor(Number(pageSequence))) : 0
  const nextVisitId = randomUUID()
  let active = false
  let status = 'ended'
  ...
  await db.runTransaction(async transaction => {
    ...
    if (!['starting', 'active'].includes(status)) return
    ...
    if (normalizedPageName) {
      const currentPageName = String(session.currentPageName || '')
      const currentVisitId = String(session.currentPageVisitId || '')
      const currentPageSequence = Number(session.currentPageSequence || 0)
      const pageUpdateIsCurrent = normalizedPageSequence === 0 || normalizedPageSequence >= currentPageSequence

      if (pageUpdateIsCurrent && (!currentVisitId || currentPageName !== normalizedPageName)) {
        if (currentVisitId) {
          transaction.set(
            db.collection(POWER_BI_PAGE_VISIT_COLLECTION).doc(currentVisitId),
            { endedAt: now, lastSeenAt: now, updatedAt: now },
            { merge: true }
          )
        }

        transaction.set(db.collection(POWER_BI_PAGE_VISIT_COLLECTION).doc(nextVisitId), {
          sessionId,
          userId: session.userId || user.uid,
          userEmail: session.userEmail || user.email || '',
          userName: session.userName || user.name || user.email || 'Usuario',
          companyId: session.companyId || company.id,
          companySlug: session.companySlug || company.slug,
          companyName: session.companyName || company.name,
          reportKey,
          reportLabel: session.reportLabel || reportKey,
          pageName: normalizedPageName,
          pageLabel: normalizedPageLabel || normalizedPageName,
          isAdminAccess: Boolean(session.isAdminAccess),
          excludeFromUsageMetrics: Boolean(session.excludeFromUsageMetrics),
          startedAt: now,
          lastSeenAt: now,
          endedAt: null,
          createdAt: now,
        ...
        updates.currentPageName = normalizedPageName
        updates.currentPageLabel = normalizedPageLabel || normalizedPageName
        updates.currentPageVisitId = nextVisitId
        updates.currentPageStartedAt = now
        updates.currentPageSequence = normalizedPageSequence || currentPageSequence + 1
      } else if (pageUpdateIsCurrent) {
        transaction.set(
          db.collection(POWER_BI_PAGE_VISIT_COLLECTION).doc(currentVisitId),
          {
            pageLabel: normalizedPageLabel || session.currentPageLabel || normalizedPageName,
            lastSeenAt: now,
            updatedAt: now,
          },
          { merge: true }
        )
        updates.currentPageLabel = normalizedPageLabel || session.currentPageLabel || normalizedPageName
        updates.currentPageSequence = Math.max(currentPageSequence, normalizedPageSequence)
      }
    }
  ...
  if (!active) return { enabled: true, active: false, status }
  await controlRef(db).set({ idleSince: null, lastSessionActivityAt: now, updatedAt: now }, { merge: true })
...
async function recordEndedSessionMetrics(transaction, db, session, endedAt) {
  if (session.metricsRecordedAt || session.excludeFromUsageMetrics) return
...
export async function endPowerBiDashboardSession({ sessionId, user, company, reportKey, reason = 'closed' }) {
  ...
  await db.runTransaction(async transaction => {
    ...
    if (session.currentPageVisitId) {
      transaction.set(
        db.collection(POWER_BI_PAGE_VISIT_COLLECTION).doc(String(session.currentPageVisitId)),
        { endedAt: now, lastSeenAt: now, updatedAt: now },
        { merge: true }
      )
    }
  ...
  let capacityAction = ''
  try {
    const reconciliation = await reconcilePowerBiCapacity()
    capacityAction = reconciliation.action || ''
  } catch (error) {
    console.error('[power-bi-capacity:close-reconcile]', error)
  }

  return { enabled: true, ended: true, capacityAction }
}
  ...
  for (const document of snapshot.docs) {
    ...
    if (!isLiveSession(session, now)) {
      await db.runTransaction(async transaction => {
        ...
        if (data.currentPageVisitId) {
          const lastSeenAt = asDate(data.lastHeartbeatAt || data.lastActivityAt) || now
          transaction.set(
            db.collection(POWER_BI_PAGE_VISIT_COLLECTION).doc(String(data.currentPageVisitId)),
            { endedAt: lastSeenAt, lastSeenAt, updatedAt: now },
            { merge: true }
          )
        }
        transaction.set(
...
export async function reconcilePowerBiCapacity() {
  ...
  const controlSnapshot = await controlRef(db).get()
  const control = controlSnapshot.exists ? controlSnapshot.data() : {}
  const existingIdleSince = asDate(control.idleSince)
  const idleSince = existingIdleSince || now
  if (!existingIdleSince) {
  ...
  if (status.state !== 'active') {
    return { configured: true, action: 'waiting-capacity-state', state: status.state, activeSessions: 0, expiredSessions }
  }
